...
## 2026-08-09 — DevCard is public with a server-rendered PNG (D-B)

Source: `design/REDESIGN-PLAN.md` §4.6 (decision D-B). Approved by Izzy 2026-08-09.

- **What shipped:** a public page at `/u/{slug}/devcard` (`apps/web/app/(community)/u/[slug]/devcard/page.tsx`), a server-rendered PNG at `/api/og/devcard/{slug}` (`apps/web/app/api/og/devcard/[slug]/route.tsx`), an exact-segment allowlist in `apps/web/middleware.ts`, and an owner-visible notice on the page saying the card is public.
- **PNG route:** `runtime = 'nodejs'` and `dynamic = 'force-dynamic'` — Drizzle needs Node and the font is read off disk. `ImageResponse` from `next/og` at 1200×630. Both Fraunces TTFs are committed under `apps/web/assets/fonts` and read via `node:fs/promises` into a module-level memoised promise, so this never becomes a request-time fetch to `fonts.googleapis.com`. Two DB queries (`getDevCardSummary`: the user row, plus one statement computing posts and accepted-solutions as scalar subqueries), both awaited before any JSX is constructed because satori renders synchronously. Unknown slug returns a plain 404, not a thrown render. Response is cached `public, max-age=0, s-maxage=3600, stale-while-revalidate=86400`.
- **Allowlist anchoring:** `PUBLIC_DEVCARD_REGEX = /^\/(?:u\/[^/]+\/devcard|api\/og\/devcard\/[^/]+)\/?$/`, evaluated in `needsProtection` after the static-asset bail-outs and before the community gate. It is anchored at **both** ends, and `[^/]+` cannot span a `/`, so the slug is pinned to exactly one segment: only `/u/{slug}/devcard` (3 segments) and `/api/og/devcard/{slug}` (4 segments) match. `/u/{slug}`, `/u/{slug}/followers`, `/u/{slug}/devcard/edit` and `/u/{a}/{b}/devcard` all fail to match.
- **Keep both anchors.** Dropping the `$`, or substituting `startsWith('/u/')` / `startsWith('/api/og/')`, converts a two-route exception into a blanket public carve-out of everything beneath those prefixes — every `/api/og/*` route present or future, and every profile sub-route added later. It would also silently pre-approve any gated sub-route someone adds under `/u/{slug}/` (followers, edit, DM entry points), with no test failing to catch it. Today's live blast radius is narrower than §4.6 assumed only because `/u/{slug}` is already public for an unrelated reason (see the entry above) — that is a coincidence of the current route set, not a reason to relax the match.
- **Owner notice:** rendered only when `isOwner` — "This DevCard is public, so anyone you send the link to can open it — no account needed. It shows the same stats as your public profile." Ownership comes from `getSession()`, a cookie decode with no DB query, so the page's query budget is unchanged.
- **Tests:** `apps/web/e2e/access-matrix.spec.ts` covers both directions — signed-out visitors get 200 on the page and an `image/png` (verified by PNG magic number) from the route, an unknown slug 404s, and `/settings`, `/messages`, `/bookmarks`, `/notifications`, `/moderation` still redirect to `/login`.
- **Known edge, not yet closed:** the middleware `config.matcher` is `'/((?!_next/static|_next/image|favicon.ico|.*\\.).*)'`, which skips any path containing a dot. A userslug containing `.` bypasses middleware entirely rather than being matched by the allowlist.

## 2026-08-09 — The five email switches (D-C)

Source: `design/REDESIGN-PLAN.md` (decision D-C). Approved by Izzy 2026-08-09.

Settings surfaces exactly five email switches (`EMAIL_SWITCHES` in `apps/web/app/(community)/components/SettingsPage.tsx`), all five declared in `packages/api/src/contracts/users.ts`. **Only two of the five drive a real send today.**

- **`weeklyDigest` — opt-IN, live.** The digest cron filters on `sql\`${schema.users.preferences}->>'weeklyDigest' = 'true'\`` in `apps/web/app/api/v1/admin/jobs/weekly-digest/route.ts`. That is a string comparison against the JSON text, so an absent value means **not sent** — genuine opt-in.
- **`emailSolutions` — opt-OUT, live.** `sendTransactional` in `apps/web/lib/email.ts` maps `solution_accepted → emailSolutions` and returns early only on `prefs[eventSwitch] === false`. Strict `=== false`, never a truthiness check, so absent/`undefined` keeps sending — genuine opt-out. (`invite_accepted` has no per-event switch and is governed by `emailNotifications` alone.)
- **`emailReplies`, `emailMentions`, `emailFollows` — stored only.** No job or service reads them; they appear solely in the contract, `SettingsPage.tsx`, and tests. The UI marks each with a `not sending yet` badge (`live: false`), and the section footnote tells the user switching one on will not put mail in their inbox yet.
- **Every switch carries a `defaultOn` mirroring backend semantics so the UI cannot lie.** `weeklyDigest` is `defaultOn: false`; the other four are `defaultOn: true` — exactly matching "absent means don't send" for the opt-in digest and "absent means send" for the opt-out transactional path. `defaultOn` feeds both the rendered state (`checked={preferences[s.key] ?? s.defaultOn}`) and the save payload, which materialises all five keys on every save. If a `defaultOn` ever drifts from its backend rule, the toggle misrepresents delivery and saving the form silently changes it.
- **`emailNotifications` remains a master kill-switch, honoured in code but no longer surfaced in the UI.** `apps/web/lib/email.ts` suppresses every transactional send on `prefs.emailNotifications === false` (again strict, default ON). It is absent from `EMAIL_SWITCHES`; values stored by the old UI are still honoured, and a PATCH of the five switches merges server-side so the stored key is preserved rather than clobbered. Net effect: an orphaned kill-switch — enforced at send time, unreachable for the user.
- Unrelated near-miss worth not confusing with the above: `DEFAULT_SETTINGS.notifications.defaultPreferences` in `apps/web/lib/services/admin.ts` contains `weeklyDigest: true`, but nothing applies `defaultPreferences` to new user rows, so that admin toggle opts nobody in. The cron still requires the per-user value.
- **Tests:** `apps/web/e2e/email-preferences.vitest.ts` asserts the matrix (master kill-switch suppresses both events; `emailNotifications: true` does not override a per-event `emailSolutions: false`; `emailSolutions: false` leaves `invite_accepted` sending). `apps/web/e2e/notifications-settings.spec.ts` asserts the three "not sending yet" badges remain.
