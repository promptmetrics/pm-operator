...
export default function ModerationPage() {
  const [flags, setFlags] = React.useState<FlagCardFlag[]>([]);
  ...
  const resolveFlag = async (id: string, resolution: 'resolved' | 'dismissed') => {
    try {
  ...
  const handleWarn = async (flag: FlagCardFlag) => {
    toast({ title: `Warning sent to ${flag.target.author.username}`, variant: 'success' });
  };

  const handleBan = async (flag: FlagCardFlag) => {
  ...
  return (
    <div className="mx-auto max-w-5xl">
                  ...
                  <div className="flex justify-end">
                    <ModerationActions
                      onView={() => {
                        ...
                        if (target.group?.slug && target.postSlug) {
                          window.open(`/g/${target.group.slug}/${target.postSlug}`, '_blank');
                        }
                      }}
                      onDismiss={() => resolveFlag(flag.id, 'dismissed')}
                      onHide={() => resolveFlag(flag.id, 'resolved')}
      ...
      <ConfirmDialog
        ...
        confirmLabel="Delete"
        onConfirm={async () => {
          if (deleteFlagId) await removeFlag(deleteFlagId);
        }}
      />
    </div>
  );
