...
3. **WS6 — Circle & profile pages** (T6.1–T6.4): circle aggregates (posts/mo, solved rate — member count + created date are spec'd and still unrendered; post-page circle card omits "posts this month" pending T6.1), circle page rebuild retaining D8 surfaces, profile rebuild (stat cards, level progress via `levelForScore()` fields, per-circle contributions), header points/level/streak cluster (data already served by `/api/v1/me` + `/api/v1/me/streak`).
...
- Ops (2026-08-01 late): production `DATABASE_URL` now uses the Supabase TRANSACTION pooler (port 6543) after a session-pool exhaustion incident (EMAXCONNSESSION — fix commits `e5b2245` CI turbopack root, `1722bff` pool idle_timeout); test-data cleanup SQL was run (patterns: `%@example.com`+`test-%` users, `{prefix}-{13-digit-ts}-` groups) — NOTE it does not yet match the `Tmp {ts}`/`tmpuser…` debris from concurrency.vitest, and every E2E/CI run against the shared DB creates fresh test rows; re-run/extend the cleanup as needed or move tests to a separate database.
